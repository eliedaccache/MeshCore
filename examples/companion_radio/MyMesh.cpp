...
void MyMesh::onChannelMessageRecv(const mesh::GroupChannel &channel, mesh::Packet *pkt, uint32_t timestamp,
  ...
  // Forward channel message to C6 via UART
  {
    ChannelDetails ch_detail;
    const char* ch_name = "unknown";
    if (getChannel(channel_idx, ch_detail) && ch_detail.name[0] != '\0') {
      ch_name = ch_detail.name;
    }
    Serial2.printf("MSG:%s:%s\n", ch_name, text);
    Serial.printf("[C6 TX] MSG:%s:%s\n", ch_name, text);
  }
